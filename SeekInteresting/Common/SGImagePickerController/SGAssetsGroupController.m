...
//    self.navigationItem.backBarButtonItem = backItem;
    ...
    UIBarButtonItem *barItem  = [[UIBarButtonItem alloc] initWithImage:[[UIImage imageNamed:@"arrow-back"] imageWithRenderingMode:UIImageRenderingModeAlwaysOriginal] style:UIBarButtonItemStylePlain target:self action:@selector(goBackAction:)];
    self.navigationItem.leftBarButtonItem = barItem;
}

- (void)goBackAction:(UIBarButtonItem *)item {
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    collectionVC.maxCountImages = self.maxCount;
    collectionVC.group = self.groups[indexPath.row];
    [self.navigationController pushViewController:collectionVC animated:YES];
}
- (NSIndexPath *)tableView:(UITableView *)tableView willSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    
    ShowProgressHUDWithText(YES, nil, @"正在加载...");
